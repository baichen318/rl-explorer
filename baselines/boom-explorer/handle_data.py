...
def handle_power():
    if if_exist(configs['pt-pwr-path']):
        prefix = "BOOMRANDOM" if configs['initialize-method'] == "random" \
        ...
        for root in configs['config-name']:
            ...
            if base:
                ...
                root = prefix + 'Config' + root.split('Config')[1] + 'Config-benchmarks'
    ...
    if method == "crted":
        ...
        for i in range(len(features)):
            ...
            for bmark in benchmarks.keys():
                ...
                for p in power:
                    _p = p[0].split('-')
                    if (c_name == _p[0].lstrip("BOOM").rstrip("Config")) and \
                        (p[0].split('benchmarks-')[-1] == bmark):
                        if not (np.isnan(p[-1]) or p[-1] == 0):
                            _bp.append(p[-1])
            _data.append(np.mean(_bl))
            _data.append(np.mean(_bp))
